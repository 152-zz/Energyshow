...
def load_lstm(data, start, tail,target_name=None):
    ...
    # 读取模型
    net = torch.load('./LSTM_240427/LSTM_oil_price2.pth')
    
    #读取数据参数
    with open('./LSTM_240427/id_set.pkl', 'rb') as f:
        ids = pickle.load(f) #读取国家ids
    
    scaler = joblib.load('./LSTM_240427/min_max_scaler.pkl')#读取label_encoder
    id_encoder = joblib.load('./LSTM_240427/label_encoder.pkl')#读取min_max_encoder
    ...
    for id1 in list(ids):
        ...
        fig_dict[id1] = plot_result(yhat.T,x.T,train_end_ind)
        # 输出结果
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('./LSTM_240427/data_Cheng.csv')
